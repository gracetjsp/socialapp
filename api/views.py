from django.shortcuts import render
from api.serializers import UserSerializer,QuestionSerializer,AnswerSerializers
from rest_framework.viewsets import ModelViewSet
from rest_framework.response import Response
from django.contrib.auth.models import User
from api.models import Questions,Answers
from rest_framework import authentication,permissions
from rest_framework.decorators import action
# Create your views here.
class UserView(ModelViewSet):
    serializer_class = UserSerializer
    queryset = User.objects.all()
class QuestionView(ModelViewSet):
    serializer_class = QuestionSerializer
    queryset = Questions.objects.all()
    #authentication
    authentication_classes = [authentication.TokenAuthentication]
    permission_classes = [permissions.IsAuthenticated]
    #for saving authentication data
    def perform_create(self, serializer):
        serializer.save(user = self.request.user)
    @action(methods = ["GET"],detail = False)
    def my_questions(self,request,*args,**kw):
        qs = request.user.questions_set.all()
        serializer = QuestionSerializer(qs,many = True)
        return Response(data = serializer.data)
    #login cheyidha aluda questions kittan
    #localhost:8000/questions/my_questions/
    @action(methods=["post"],detail= True)
    def add_answer(self,request,*args,**kw):
        id = kw.get('pk')
        qustn = Questions.objects.get(id =id)
        usr = request.user
        serializer = AnswerSerializers(data = request.data,context = {"question":qustn,"user":usr})
        if serializer.is_valid():
            serializer.save()
            return Response(data=serializer.data)
        else:
            return Response(data=serializer.errors)
    @action (methods=['get'],detail= True)
    def list_answers(self,request,*args,**kw):
            id = kw.get("pk")
            ques = Questions.objects.get(id = id)
            qs = ques.answers_set.all()
            serializer = AnswerSerializers(qs,many = True)
            return Response(data= serializer.data)

#localhost:8000/questions/1/add_answer/
#localhost:8000/questions/1/list_answers
#for upvote
class AnswersView(ModelViewSet):
    serializer_class = AnswerSerializers
    queryset = Answers.objects.all()
    authentication_classes =[authentication.TokenAuthentication]
    permission_classes = [permissions.IsAuthenticated]
    #localhost:8000/answer/1/upvote
    @action(methods=['get'],detail= True)
    def upvote(self,request,*args,**kw):
        ans = self.get_object()#answer object
        usr = request.user
        ans.up_vote.add(usr)
        return Response(data="created")

